...
describe('generateKey', () => {
	it('produces a PREFIX-grouped key with a 16-char body (PRD §10, 78 bits)', async () => {
	...
	it('only uses alphabet characters (no ambiguous I L O U 0 1)', async () => {
		for (let i = 0; i < 200; i++) {
	...
	it('is effectively unique across many draws', async () => {
		const seen = new Set<string>();
		for (let i = 0; i < 5000; i++) seen.add(generateKeyBody());
		expect(seen.size).toBe(5000);
	});

	it('draws each alphabet symbol with no gross bias', async () => {
...
describe('parseKey / normalization', () => {
	it('round-trips display format to stored format', async () => {
	...
	it('normalizes whitespace, lowercase, and stray dashes', async () => {
		expect(parseKey('  clem-a2b3c4d5-e6f7g8h9  ', 'CLEM')?.normalized).toBe('CLEMA2B3C4D5E6F7G8H9');
	});

	it('rejects the wrong prefix', async () => {
	...
	it('rejects excluded ambiguous characters', async () => {
	...
	it('normalizedKey composes prefix and body', async () => {
