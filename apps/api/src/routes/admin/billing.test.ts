...
describe('POST /admin/billing/checkout', () => {
	it('creates a customer once and reuses it', async () => {
		const h = harness();
		const first = await h.app.request('/admin/billing/checkout', {
			method: 'POST',
		...
		await h.app.request('/admin/billing/checkout', { method: 'POST', headers: h.adminHeaders });
	...
	it('stamps the account on both the session and the subscription', async () => {
		...
		expect(gateway.checkouts[0]).toMatchObject({ accountId: 1, priceId: PRO_PRICE });
	});
	...
	it('sends an already-subscribed account to the portal instead', async () => {
		...
		h.deps.db.$client
			.prepare(
				"UPDATE account_subscriptions SET stripe_subscription_id = 'sub_1' WHERE account_id = 1",
			)
			.run();
