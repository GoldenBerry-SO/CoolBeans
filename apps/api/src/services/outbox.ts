...
export function enqueue(deps: AppDeps, kind: JobKind, payload: object, delayMs = 0): void {
	const runAfter = new Date(nowDate(deps).getTime() + delayMs).toISOString();
	deps.db
		.insert(outbox)
		.values({ kind, payload: JSON.stringify(payload), runAfter })
		.run();
}
...
function markDone(deps: AppDeps, id: number): void {
	deps.db
		.update(outbox)
...
function markFailed(deps: AppDeps, id: number, error: string, attempts: number): void {
...
export async function processJob(
	...
	try {
		...
		markDone(deps, job.id);
	} catch (err) {
		markFailed(deps, job.id, err instanceof Error ? err.message : String(err), job.attempts + 1);
	}
}
