...
export function agentPrompt(product: Product, baseUrl: string): string {
	...
	return [
		...
		'   unknown key) keeps the last known-good state, so never lock a paying user out for one.',
		'   The one exception is a fresh install with nothing cached, which is deny/uninitialized:',
		'   ask for a key, never phrase it as a revocation.',
...
export function configFacts(product: Product, baseUrl: string): ConfigFact[] {
	...
	return [
		...
		{ label: 'Product slug', value: product.slug, mono: true },
		{
			...
			mono: true,
			hint: `Keys look like ${product.keyPrefix}-XXXX-XXXX-XXXX-XXXX.`,
		},
		{
			label: 'Seat model',
