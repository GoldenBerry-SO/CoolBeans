...
- Answer any 2xx quickly (do your work after responding). Anything else counts as a failure
  and is retried.
- The delivery log (console → Webhooks → Deliveries) shows status, attempts, and the last
  error per delivery.
...
keep using the client SDK contract — activate, validate, offline tokens — and never gate a
